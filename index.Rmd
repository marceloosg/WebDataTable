...
print(paste("Title:",db[[1]]))
price=as.numeric(gsub("\\$","",as.character(db[[2]]$`Price(USD)`)))
db[[2]]$`Price(USD)`=price
colnames(db[[2]])=gsub("\\(.*","",colnames(db[[2]]))
head(db[[2]][!is.na(price),c(1,2,3,4,5)],5)
```
---
